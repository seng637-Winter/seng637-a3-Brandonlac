...
// Test made for getUpperBound, had to force the upper bound to be smaller number than lower.
	@Test(expected = IllegalArgumentException.class)
    public void testIllegalGetUpperBound() throws NoSuchFieldException, IllegalAccessException {
        Range range = new Range(1, 10); // A normal range

        // Use reflection to change the internal state of the range object
        ...
        // Set the lower bound to a value greater than the upper bound
        lowerField.setDouble(range, 20);
        upperField.setDouble(range, 10);

        // This should throw an IllegalArgumentException
        range.getUpperBound();
    }
	
// Test made for getLowerBound, had to force the upper bound to be smaller number than lower.
    @Test(expected = IllegalArgumentException.class)
    public void testIllegalGetLowerBound() throws NoSuchFieldException, IllegalAccessException {
        Range range = new Range(1, 10); // A normal range
...
// Test made for getLength, had to force the upper bound to be smaller number than lower.
    @Test(expected = IllegalArgumentException.class)
    public void testGetLengthWithInvertedBounds() throws Exception {
        Range range = new Range(1, 2); // Create a valid range.

        // Use reflection to invert the range bounds.
        Field lowerField = Range.class.getDeclaredField("lower");
        Field upperField = Range.class.getDeclaredField("upper");
        lowerField.setAccessible(true);
        upperField.setAccessible(true);

        // Set the 'lower' field to a value greater than 'upper'.
        lowerField.setDouble(range, 3);
        upperField.setDouble(range, 2);

        // Attempt to get the length, which should throw an IllegalArgumentException.
        range.getLength();
    }
